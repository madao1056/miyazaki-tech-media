...
export function calculateReadingTime(content: string): number {
  ...
  const englishWords = plainText.replace(/[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]/g, '').split(/\s+/).filter(word => word.length > 0).length;
  
  // 日本語: 500文字/分、英語: 250単語/分
  const readingTimeMinutes = Math.ceil((japaneseChars / 500) + (englishWords / 250));
...
// カテゴリーIDからスラッグを取得
export function getCategorySlug(categoryId: number, categories: Category[]): string {
...
export function generateExcerpt(content: string, maxLength: number = 120): string {
  ...
  const lastJapaneseChar = truncated.match(/[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF](?=[\s\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]*$)/);
...
// 画像URLを取得
export function getFeaturedImageUrl(post: Post, size: string = 'medium_large'): string | null {
...
// 著者情報を取得
export function getAuthorInfo(post: Post): { name: string; avatar?: string } | null {
